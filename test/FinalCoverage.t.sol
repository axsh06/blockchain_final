...
contract FinalCoverageTest is Test {
    ...
    function test_GovName() public view {
        assertEq(governor.name(), "ProtocolGovernor");
    }

    function test_GovVersion() public view {
        assertEq(governor.version(), "1");
    }

    function test_GovDelay() public view {
        assertEq(governor.votingDelay(), 86400);
    }

    function test_GovPeriod() public view {
        assertEq(governor.votingPeriod(), 604800);
    }

    function test_GovThreshold() public view {
    ...
    function test_GovTokenAddress() public view {
        assertEq(address(governor.token()), address(token));
    }

    function test_GovTimelockAddress() public view {
    ...
    function test_TimeDelay() public view {
        assertEq(timelock.getMinDelay(), 172800);
    }

    function test_TimeAdminRole() public view {
        assertTrue(timelock.hasRole(timelock.DEFAULT_ADMIN_ROLE(), admin));
    }

    function test_TimeProposerRole() public view {
        assertTrue(timelock.hasRole(timelock.PROPOSER_ROLE(), admin));
    }

    function test_TimeExecutorRole() public view {
    ...
    function test_VaultMaxWithdraw() public view {
        assertEq(vault.maxWithdraw(user), 0);
    }

    function test_VaultMaxRedeem() public view {
        assertEq(vault.maxRedeem(user), 0);
    }

    function test_VaultPreviewDeposit() public view {
        assertEq(vault.previewDeposit(100), 100);
    }

    function test_VaultPreviewMint() public view {
        assertEq(vault.previewMint(100), 100);
    }

    function test_VaultPreviewWithdraw() public view {
        assertEq(vault.previewWithdraw(100), 100);
    }

    function test_VaultPreviewRedeem() public view {
        assertEq(vault.previewRedeem(100), 100);
    }

    function test_TokenDelegatesEmpty() public view {
